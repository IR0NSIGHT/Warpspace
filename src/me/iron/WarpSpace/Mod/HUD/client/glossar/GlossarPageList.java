...
public class GlossarPageList extends GUIAncor {
    private GUIElement autoWrap;
    public GlossarPageList(float v, float v1, GUIElement guiElement, InputState inputState) {
    ...
    @Override
    public void onInit() {
        GUIScrollablePanel pageOverview = new GUIScrollablePanel(getWidth()*0.2f,getHeight(),this, getState());
        pageOverview.setContent(new GUIColoredRectangle(getState(),pageOverview.getWidth(),pageOverview.getHeight(),pageOverview,new Vector4f(1,1,0,0)));
        pageOverview.setScrollable(GUIScrollablePanel.SCROLLABLE_VERTICAL);
        attach(pageOverview);

        GlossarPage testpage = new GlossarPage(getState(), autoWrap ,(int)(getWidth()-pageOverview.getWidth()),(int)getHeight(),"WarpSpace","an alternative way to faster-than-light travel in starmade.\n" +
                "\n" +
        ...
        testpage.setPos(pageOverview.getWidth(),0,0);
        attach(testpage);
        super.onInit();
