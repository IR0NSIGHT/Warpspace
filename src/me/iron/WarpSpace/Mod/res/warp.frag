...
void main()
{
    ...
    bgRingFunc = mix(bgRingFunc,0.,min(1.0,abs(dotVel)*mix(5.0,2.0,flightSpeed))); //rings fade out ahead and behind//END OLD ITHI VARIABLES
    ...
    float theta = planeAngle(/*plane 1*/flightVel,refY,/*plane 2*/flightVel,vertPos); //TODO: This math in planeAngle is wrong. I end up getting coordinates that mirror... //TODO: I suspect I accidentally made a setup that doesn't respect angles above pi //theta *= 0.5;
    ...
    float maximumPower = mix(10.0,2.0,flightSpeed);
    r = pow(r,mix(maximumPower,1.0,pow(1.-r,flightSpeed)));
    ...
    finalColor.r = finalColor.r * (1.0 - (0.1*flightSpeed));
    ...
    float tunnelEndsExp = mix(2100.0,1984.0,flightSpeed*flightSpeed);
    ...
    float staticVFX = max(0.0,1.0 - ithnoise(vertPos,speedMixture,time));
    ...
    gl_FragColor = vec4(finalColor, 1.0);//vec4(c*color.rgb, 1.0);
}
