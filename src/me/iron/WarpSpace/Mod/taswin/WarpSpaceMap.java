	...
	public static void enable(final StarMod instance)
	{
		...
		StarLoader.registerListener(GalaxyFinishedGeneratingEvent.class, new Listener<GalaxyFinishedGeneratingEvent>()
		{
			...
			public void onEvent(GalaxyFinishedGeneratingEvent event)
			{
				...
				{
					final Vector4f color = galaxy.getSunColor(new Vector3i(starPoses.get(i)));
					final int subSprite = galaxy.getSystemType(new Vector3i(starPoses.get(i)));

					Vector3f systemPos = starPoses.get(i);
					...
					for (PlayerState player : GameServer.getServerState().getPlayerStatesByName().values())
					{
						...
						if (currentControl instanceof SegmentController)
						{
							...
							if (Math.abs(sectorPos.y) >= WarpManager.universeHalfSize)
							{
								...
								int distanceOverBorder = Math.abs(sectorPos.y) - WarpManager.universeHalfSize;
								float maxSpeed = ((SegmentController)currentControl).getMaxServerSpeed();
								
								for (Sendable s : sendables)
								{
									RigidBody rigidBody = (RigidBody) ((SegmentController) s).getPhysicsDataContainer().getObject();
									Vector3f linearVelocity = new Vector3f(rigidBody.getLinearVelocity(new Vector3f()));
									
									if (distanceOverBorder > 0)
										linearVelocity.y = 10 * maxSpeed * (sectorPos.y >= 0 ? -1 : 1);
									else if (Math.abs(linearVelocity.y) > maxSpeed || (linearVelocity.y * (sectorPos.y >= 0 ? 1 : -1) > 0))
										linearVelocity.y = 0;
									rigidBody.setLinearVelocity(linearVelocity);
								}
								player.sendServerMessage(new Object[] {((SegmentController) currentControl).getPhysicsObject().getCenterOfMassPosition(new Vector3f())}, 1);
								if (distanceOverBorder > 0)
									player.sendServerMessage(new Object[] {"You are out of bounds pushing you back in."}, 3);
								else
									player.sendServerMessage(new Object[] {"You have reached the world boarder stop!."}, 2);
							}
						}
						/*else if (!player.isControllingCore())
						{
							PlayerCharacter pc = player.getAssingedPlayerCharacter();
							if (pc == null) return;
							RigidBody rigidBody = (RigidBody)pc.getPhysicsDataContainer().getObject();
							if (rigidBody == null) return;
							Sendable[] sendables = SegmentControllerUtils.getDualSidedSendable((Sendable) rigidBody);
							for (Sendable s : sendables)
							{
								((RigidBody)s).setLinearVelocity(new Vector3f(0, 10, 0));
							}
						}*/
					}
				}
